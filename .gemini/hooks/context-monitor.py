...
def get_session_dir() -> Path:
    ...
    if not project_dir:
        return Path.home() / ".gemini" / "sessions" / "default"

    import hashlib
    project_hash = hashlib.md5(project_dir.encode()).hexdigest()[:8]
    session_dir = Path.home() / ".gemini" / "sessions" / project_hash
...
def estimate_context_percentage() -> float:
    ...
    This is a heuristic since we don't have direct access to Gemini's context
...
Skills are saved to {MAGENTA}.gemini/skills/{NC} and persist across sessions.
"""
...
def run_context_monitor() -> int:
    ...
    if percentage >= THRESHOLD_CRITICAL and not shown["warn_90"]:
        ...
        return 0  # Non-blocking warning (exit 2 would block Gemini)
