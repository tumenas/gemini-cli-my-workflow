#!/usr/bin/env python3
"""
Session Log Reminder Hook for Gemini CLI

A Stop hook that tracks how many responses have passed since the session
log was last updated, and nudges Gemini to update the log via stderr
...
{"decision": "block"} to stop Gemini mid-turn. That was effective but
...
Usage (in .gemini/settings.json):
    "Stop": [{ "hooks": [{ "type": "command", "command": "python3 \"$CLAUDE_PROJECT_DIR\"/.gemini/hooks/log-reminder.py" }] }]
"""
...
def get_state_dir() -> Path:
    """Get state directory under ~/.gemini/sessions/ keyed by project."""
    import os
    project_dir = os.environ.get("CLAUDE_PROJECT_DIR", "")
    if not project_dir:
        state_dir = Path.home() / ".gemini" / "sessions" / "default"
    else:
        project_hash = hashlib.md5(project_dir.encode()).hexdigest()[:8]
        state_dir = Path.home() / ".gemini" / "sessions" / project_hash
...
def get_project_dir():
    ...
    # If stop_hook_active, Gemini is already continuing from a previous
...
if __name__ == "__main__":
    try:
        ...
        # Fail open — never block Gemini due to a hook bug
        sys.exit(0)
