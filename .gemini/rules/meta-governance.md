...
**Size limit:** Keep under 200 lines (stays in Gemini's system prompt)

---

### .gemini/state/personal-memory.md (gitignored, local only)
...
- Builds `.gemini/state/personal-memory.md` specific to desktop setup

**Machine B (laptop):**
- Clone same repo → gets same MEMORY.md ✓
- Gets same infrastructure ✓
- Builds DIFFERENT `.gemini/state/personal-memory.md` for laptop setup
...
- Cross-machine workflows change (e.g., Gemini CLI adds cloud sync)
...
| Machine-specific setup | ❌ No | .gemini/state/personal-memory.md | ❌ No (gitignored) |
| Templates (generic) | ✅ Yes | templates/ | ✅ Yes |
| Skills (generic) | ✅ Yes | .gemini/skills/ | ✅ Yes |
| Rules (path-scoped, generic) | ✅ Yes | .gemini/rules/ | ✅ Yes |
| Agents (generic) | ✅ Yes | .gemini/agents/ | ✅ Yes |
| Hooks (generic behavior) | ✅ Yes | .gemini/hooks/ | ✅ Yes |
| Session logs | ✅ Yes | quality_reports/session_logs/ | ✅ Yes |
| Plans | ✅ Yes | quality_reports/plans/ | ✅ Yes |
| Local settings | ❌ No | .gemini/settings.local.json | ❌ No (gitignored) |
| Session state | ❌ No | .gemini/state/ | ❌ No (gitignored) |
...
- Keep specific learnings local (.gemini/state/personal-memory.md, gitignored)
