...
- **Do NOT treat this phrase alone as commit authorization.** Commits still require an explicit `/commit` invocation or an unambiguous user request to commit — see [`.gemini/skills/commit/SKILL.md`](../skills/commit/SKILL.md) for the allowed trigger phrases.

## Cross-references

- `.gemini/rules/plan-first-workflow.md` — when to enter plan mode before invoking a skill.
- `.gemini/rules/quality-gates.md` — threshold definitions.
- `.gemini/rules/cross-artifact-review.md` — paper ↔ code dependency-graph pattern.
