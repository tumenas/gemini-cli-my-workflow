...
**This is a collaborative, iterative process. The instructor drives the vision; Gemini is a thinking partner.**

---

## CONSTRAINTS (Non-Negotiable)

1. **read_file the knowledge base FIRST** — notation registry, narrative arc, applications database
...
read_file the inputs, then produce a Pre-Flight Report in your response before Phase 1 starts.

Inputs to read:
- `.gemini/rules/knowledge-base-template.md` — notation registry, narrative arc, applications
- `.gemini/rules/content-invariants.md` — INV-1..INV-8 govern slide content
...
**First-lecture fallback (fresh fork, empty knowledge base).** If `.gemini/rules/knowledge-base-template.md` still has unfilled placeholder tables (no notation registry entries, no applications, no prior lectures in `Slides/`), do NOT halt waiting for it. Instead:
...
3. write_file the approved stub into the knowledge base template so subsequent lectures inherit it.
