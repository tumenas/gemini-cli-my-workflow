...
1. Frontmatter `allowed-tools` ↔ body tool-invocation parity (e.g. body spawns `codebase_investigator` but `codebase_investigator` not in `allowed-tools` — the v1.7.0 PR #92 miss).
...
Launch these 4 agents simultaneously using `codebase_investigator` with `subagent_type=general-purpose`. Each agent's prompt **must** tell it to read `.gemini/references/audit-pet-peeves.md` and explicitly check for each class of bug before reporting clean. The pet-peeves file is a living catalogue of drift patterns review bots have caught; it grows with each PR.
...
Focus: **all** executable code in the repo — `.gemini/hooks/*.py`, `.gemini/hooks/*.sh`, `scripts/*.py`, `scripts/*.sh`, `.gemini/scripts/*.sh`. Not just `.gemini/hooks/` — when PR #93 added new code under `scripts/`, the original narrow scope meant Copilot + Codex caught 5 bugs the audit missed.

Hook-specific checks (Stop/PreToolUse/SessionStart protocols, `CLAUDE_PROJECT_DIR` usage, hash-length consistency) apply only to `.gemini/hooks/`. Everything below applies to ALL executable code:

- No remaining `/tmp/` usage in anything that manages state (should use `~/.gemini/sessions/`)
- Hash length consistency (`[:8]` across all hooks) [hooks only]
- Proper error handling — **fail-open pattern** where the docstring promises it (top-level `try/except` with `sys.exit(0)`). Python `read_text()` must catch `UnicodeError` (not just `OSError`) if the script is promised fail-open for corrupt files. run_shell_command `set -u` without `set -e` or explicit post-command checks does NOT catch command failures — verify.
...
Focus: `.gemini/skills/*/SKILL.md` and `.gemini/rules/*.md`
- Valid YAML frontmatter in all files
- No stale `disable-model-invocation: true`
- `allowed-tools` values are sensible
- **`allowed-tools` actually covers every tool the skill body invokes.** For every `codebase_investigator` spawn, `run_shell_command` command, `write_file`/`replace` call mentioned in the skill's Steps / Phases / Workflow body, verify the tool appears in the `allowed-tools` array. Common miss: skill body says "spawn `agent-X` via `codebase_investigator` with `context=fork`" but `codebase_investigator` is absent from `allowed-tools` — runtime permission error or silent bypass. Caught this class of bug after Codex/Copilot flagged it on PR #92 (4 skills promised `codebase_investigator` in their Post-Flight sections but 3 of 4 had no `codebase_investigator` permission).
...
- GEMINI.md skills table matches actual skill directories 1:1
...
- `allowed-tools` linter warning — known linter bug (Gemini CLI issue #25380), field IS valid
...
- `log-reminder.py` outputting `{"decision": "block"}` with `sys.exit(0)` — this IS the modern Gemini CLI Stop-hook block protocol, NOT a bug
...
1. read_file the file first (required by replace tool)
2. Apply the fix
...
| State in /tmp/ | All Python hooks | Should use `~/.gemini/sessions/<hash>/` |
