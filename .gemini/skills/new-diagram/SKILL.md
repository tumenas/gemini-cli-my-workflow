...
allowed-tools: ["read_file", "write_file", "replace", "run_shell_command", "glob", "codebase_investigator"]
effort: high
---
...
### Step 3: replace the content to fit the user's intent

Ask the user what the diagram should show. replace `$DST` with the `replace` tool:
...
Spawn the `tikz-reviewer` agent with `codebase_investigator` (`subagent_type=tikz-reviewer`). Pass the `.tex` source and the compiled `.pdf` path. The reviewer is now required to cite the pass and formula from [`tikz-measurement.md`](../../rules/tikz-measurement.md) for every CRITICAL/MAJOR finding — vague reports are rejected.
...
- [`.gemini/rules/tikz-prevention.md`](../../rules/tikz-prevention.md) — the P1–P6 authoring rules.
- [`.gemini/rules/tikz-measurement.md`](../../rules/tikz-measurement.md) — the six-pass protocol with formulas the reviewer cites.
- [`.gemini/rules/tikz-visual-quality.md`](../../rules/tikz-visual-quality.md) — general visual standards.
- [`.gemini/skills/extract-tikz/SKILL.md`](../extract-tikz/SKILL.md) — for pulling TikZ out of an existing Beamer deck instead of creating new.
