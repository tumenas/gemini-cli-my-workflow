...
description: Diagnose why Gemini CLI is (or isn't) prompting for permission. By default reads only repo-local layers (CLI project, CLI project-local, VSCode workspace). Host-global layers (CLI user `~/.gemini/`, VSCode user settings) are read ONLY when the user explicitly confirms — those files may contain unrelated paths or secrets. Use when user says "why is it asking me to approve?", "permission check", "why am I getting prompts?", "bypass isn't working", "check my permissions". read_file-only diagnostic.
...
Surface the full permission-mode picture across every layer Gemini CLI honors, so the user can see at a glance why prompts are (or aren't) firing. Gemini CLI resolves permission mode from a 6-tier stack; a single misconfigured layer produces silent overrides that are hard to debug by eye.

## The 6 layers (precedence: bottom wins)

1. **VSCode user settings** — `~/Library/Application Support/Code/User/settings.json` (macOS), `%APPDATA%/Code/User/settings.json` (Windows), `~/.config/Code/User/settings.json` (Linux). Key: `geminiCode.initialPermissionMode`.
2. **VSCode workspace settings** — `<repo>/.vscode/settings.json`. Same key. Wins over user.
3. **CLI user settings** — `~/.gemini/settings.json`. Key: `permissions.defaultMode`.
4. **CLI project settings** — `<repo>/.gemini/settings.json`. Same key. Wins over user.
5. **CLI project-local settings** — `<repo>/.gemini/settings.local.json`. Same key. Wins over project.
...
Host-global settings files (`~/.gemini/settings.json`, VSCode user settings) may contain:
...
This skill is designed for defense-in-depth: **Phase A runs automatically and reads only repo-local files.** Phase B reads host-global files **only after the user explicitly confirms** — never silently. When reporting host-global layers, redact any key that is not directly relevant to `permissions.*` or `geminiCode.*`.

## Protocol

### Phase A: Repo-local layers (auto-runs)

read_file these immediately — they are checked into (or gitignored inside) the repo and do not cross the trust boundary:

```bash
VSCODE_WS="${CLAUDE_PROJECT_DIR}/.vscode/settings.json"
CLI_PROJECT="${CLAUDE_PROJECT_DIR}/.gemini/settings.json"
CLI_LOCAL="${CLAUDE_PROJECT_DIR}/.gemini/settings.local.json"
```

For each file that exists, extract:
- **VSCode workspace:** `geminiCode.initialPermissionMode`, `geminiCode.allowDangerouslySkipPermissions`
...
> - `~/.gemini/settings.json` (CLI user-level)
> - your VSCode user settings (`~/Library/Application Support/Code/User/settings.json` on macOS; Linux/Windows vary)
>
> These may contain unrelated paths or secrets. I will redact any key that isn't in `permissions.*` or `geminiCode.*`. Proceed?"
...
CLI_USER="${HOME}/.gemini/settings.json"
```
...
- VSCode user: any key starting with `geminiCode.`
...
The live in-session mode is exposed via the status line (see `.gemini/scripts/statusline.sh`). Tell the user:

> "Your status line shows the current in-session mode in the top-right of the Gemini CLI panel. If that disagrees with the resolved `defaultMode` above, you (or Shift+Tab) overrode it mid-session. Press Shift+Tab to cycle back."

If the status line isn't configured, emit a warning and point at `.gemini/scripts/statusline.sh`.
...
2. **VSCode-only bypass:** VSCode layers say bypass but no CLI layer does → terminal Gemini CLI will still prompt; extension may or may not.
3. **Empty allowlist + default mode:** `defaultMode: "default"` with empty `allow` → every tool prompts, as designed.
4. **Stale session:** settings are correct but user reports prompts → almost always a session that pre-dates the fix. Advise "Cmd+Shift+P → Developer: Reload Window, then new Gemini CLI session."
...
Layer 4 — CLI project:       bypassPermissions      (allow: ["replace(**)", "run_shell_command(*)", ...])
Layer 5 — CLI project-local: bypassPermissions      (allow: [...], deny: [])

Resolved defaultMode: bypassPermissions (set by Layer 5)
Merged allow:         replace(**), write_file(**), run_shell_command(*), ...
Merged deny:          (none)

=== RUNTIME ===

Check the status line at the top of the Gemini CLI panel. Expected: [BYPASS].
