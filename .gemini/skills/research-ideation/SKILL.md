...
1. **Understand the input.** read_file `$ARGUMENTS` and any referenced files. Check `master_supporting_docs/` for related papers. Check `.gemini/rules/` for domain conventions.
...
Before returning the ideation report, run the Post-Flight Verification protocol from [`.gemini/rules/post-flight-verification.md`](../../rules/post-flight-verification.md). Research ideation is hallucination-prone in three specific ways:
...
3. **Spawn `claim-verifier`** via `codebase_investigator` with `subagent_type=claim-verifier` and `context=fork`. Hand it claims + questions + source pointers (WebSearch allowed, NBER/SSRN URLs preferred, dataset codebooks preferred). Do NOT include the draft.
