...
allowed-tools: ["read_file", "write_file", "replace", "grep_search", "glob", "run_shell_command", "codebase_investigator"]
effort: high
---
...
| --- | --- |
| `.tex`, `.qmd`, `.md`, `.txt` | read_file directly with the `read_file` tool. |
| `.pdf` | `TMP=$(mktemp --suffix=.txt) && pdftotext "FILE" "$TMP"` (poppler-utils; use `mktemp -t ...` on macOS if `--suffix` is unsupported). grep_search `"$TMP"`. |
| `.docx` | `TMP=$(mktemp --suffix=.txt) && pandoc "FILE" -t plain -o "$TMP"` (or `docx2txt "FILE" "$TMP"`). grep_search `"$TMP"`. |
| `.html` | `TMP=$(mktemp --suffix=.txt) && pandoc "FILE" -t plain -o "$TMP"`. grep_search `"$TMP"`. |
...
1. read_file the report end-to-end.
...
2. `grep_search` the **plain-text version** of the revised manuscript for those terms (and synonyms). Note: grep_search only works on text — if the original was any non-text format (for example, `.pdf`, `.docx`, or `.html`), grep the converted temp file from Step 0.
3. `read_file` the surrounding context (±20 lines) to confirm the change addresses the concern.
...
write_file the output to `response-to-referees.md` (matching the template filename) or a path the user specifies. Use the structure in `templates/response-to-referees.md`:
...
3. **Spawn `claim-verifier`** via `codebase_investigator` with `subagent_type=claim-verifier` and `context=fork`. Hand it: the claims table, the verification questions, the path to the revised manuscript. Do NOT include the response draft.
