...
- **Context loss between sessions.** You pick up where you left off, but Gemini doesn't remember *why* you chose that notation, *what* the instructor approved, or *which* bugs were fixed last time.
...
| CLI/headless mode | Run from scripts: `gemini -p "compile all lectures"` |
...
You: "Yes"
     ↓
Gemini runs /commit (only because you explicitly approved)
```
...
2. **Automatically** --- Gemini invokes them when relevant to your request

Most of the time, you just describe what you want and Gemini handles the rest. Explicit skill invocation is there when you want precise control.

::: {.callout-important}
## The Bottom Line

**You talk, Gemini orchestrates.** The 14 agents, 28 skills, and 24 rules exist so you don't have to think about them. Describe your goal, approve the plan, and let the system work.
:::

::: {.callout-note}
## You Don't Need All of This on Day One

This guide describes the full system --- 14 agents, 28 skills, 24 rules. That is the ceiling, not the floor. **Start with just GEMINI.md and 2--3 skills** (`/compile-latex`, `/proofread`, `/commit`). Add rules and agents as you discover what you need. The template is designed for progressive adoption: fork it, fill in the placeholders, and start working. Everything else is there when you're ready.
:::
...
You need three things: install Gemini CLI, fork the repo, and paste a prompt. Gemini handles everything else.

## Prerequisites {#sec-prerequisites}

| Requirement | What It Is | How to Get It |
|-------------|-----------|---------------|
| **Gemini CLI** | The AI tool that powers this workflow | `curl -fsSL https://gemini.ai/install.sh | bash` ([docs](https://code.gemini.com/docs/en/overview)) |
| **Node.js 18+** | Required to run Gemini CLI | [nodejs.org](https://nodejs.org) |
| **Gemini account** | Authentication for Gemini | [Gemini Pro or Max subscription](https://gemini.ai) or [Anthropic API key](https://console.anthropic.com) (pay per token) |
...
**Gemini Pro or Max subscription**: Includes Gemini CLI usage with generous limits. Check [gemini.ai](https://gemini.ai) for current pricing and tier details. Best for most academics.
...
Gemini CLI itself is free and open source. You pay only for the Gemini model usage.
:::

::: {.callout-tip}
## Day 1 Checklist

- [ ] Install Gemini CLI: `curl -fsSL https://gemini.ai/install.sh | bash`
- [ ] Fork the repo and clone it locally
- [ ] Run `gemini` in the project directory (first run will prompt for authentication)
- [ ] Paste the starter prompt (fill in your project details)
- [ ] Wait for Gemini to customize `GEMINI.md` for your project
- [ ] Approve the configuration plan
- [ ] Ask Gemini to do something: *"Review my slides"* or *"Create a lecture on [topic]"*
...
**"command not found: gemini":** Install Gemini CLI first: `curl -fsSL https://gemini.ai/install.sh | bash`. Requires Node.js 18+ (`node --version` to check).

**Gemini seems to ignore the configuration files:** Make sure you ran `gemini` from inside the project directory (not a parent folder). Gemini reads `.gemini/` and `GEMINI.md` from the current working directory.

**Hooks not firing (no notifications, no reminders):** Check that Python 3 is installed (`python3 --version`) and hook files are executable (`chmod +x .gemini/hooks/*`).

**"What does plan approval look like?"** Gemini presents a numbered plan and asks for your input. Say "approved", "looks good", or "revise step 3". That's it --- no special commands needed.
...
## Step 2: Start Gemini CLI and Paste This Prompt {#sec-first-session}

Open your terminal in the project directory, run `gemini`, and paste the following. Fill in the **bolded placeholders** with your project details:

::: {.callout-note collapse="true"}
## Using VS Code or Gemini Desktop instead of the terminal?

Everything in this guide works the same in any Gemini CLI interface. In **VS Code**, open the Gemini CLI panel (click the Gemini icon in the sidebar or press Cmd+Shift+P → "Gemini CLI: Open"). In **Gemini Desktop**, open your project folder and start a local session. Then paste the starter prompt below.
...
I've set up the Gemini CLI academic workflow (forked from `pedrohcgs/gemini-code-my-workflow`). The configuration files are already in this repo (`.gemini/`, `GEMINI.md`, templates, scripts). Please read them, understand the workflow, and then **update all configuration files to fit my project** --- fill in placeholders in `GEMINI.md`, adjust rules if needed, and propose any customizations specific to my use case.
...
**What this does:** Gemini will read `GEMINI.md` and all the rules, fill in your project name, institution, Beamer environments, CSS classes, and project state table, then propose any rule adjustments for your specific use case. You approve the plan, and Gemini handles the rest. From there, you just describe what you want to build.

## Optional: Manual Setup

If you prefer to configure things yourself instead of letting Gemini handle it:

::: {.callout-note collapse="true"}
## Manual Configuration Steps (click to expand)

**Customize GEMINI.md** --- Open `GEMINI.md` and replace all `[BRACKETED PLACEHOLDERS]`:
...
**Create your knowledge base** --- Open `.gemini/rules/knowledge-base-template.md` and fill in:
...
**Configure permissions** --- Review `.gemini/settings.json`. The template includes permissions for git, LaTeX, Quarto, R, and utility scripts. Add any additional tools you use.
...
For complex or ambiguous tasks, Gemini may ask 3-5 clarifying questions to create a requirements specification before planning. This catches ambiguity early and reduces rework.
...
After 3-5 questions, Gemini creates a specification document in `quality_reports/specs/` with:
...
You approve the spec, then Gemini plans implementation. This reduces mid-plan pivots by 30-50%.
...
To customize, open `.gemini/agents/domain-reviewer.md` and fill in:
...
Understanding the configuration layers helps you customize the workflow and debug when things go wrong. Gemini CLI's power comes from five configuration layers that work together --- think of them as the operating system for your academic project.
...
Move everything else into `.gemini/rules/` files (with path-scoping so they only load when relevant).
...
GEMINI.md loads every session. If it exceeds ~150 lines, Gemini starts ignoring rules silently. Put detailed standards in path-scoped rules (`.gemini/rules/`) instead --- they only load when Gemini works on matching files, so they don't compete for attention.
:::

## Rules --- Domain Knowledge That Auto-Loads {#rules---domain-knowledge-that-auto-loads}

Rules are markdown files in `.gemini/rules/` that Gemini loads automatically. They encode your project's standards. The key design principle is **path-scoping**: rules with a `paths:` YAML frontmatter only load when Gemini works on matching files.
...
Skills are multi-step workflows invoked with `/command`. Each skill lives in `.gemini/skills/[name]/SKILL.md`:
...
Gemini CLI ships with built-in skills beyond this template's 28: `/batch` orchestrates parallel refactoring across your codebase (using git worktrees for isolation), `/simplify` runs 3-agent code review and applies fixes, and `/debug` helps troubleshoot sessions. These complement the academic skills above.
:::
...
Gemini CLI also offers experimental **Agent Teams** --- multiple independent sessions that coordinate, share findings, and challenge each other's approaches. This is a research preview feature; the orchestrator + subagent pattern described here is more mature for academic workflows.
:::
...
`.gemini/settings.json` controls what Gemini is allowed to do. Here is a simplified excerpt --- the template includes additional permission entries for git, GitHub CLI, PDF tools, and more:
    ...
    "Stop": [
      {
        "hooks": [{
          "type": "command",
          "command": "python3 \"$CLAUDE_PROJECT_DIR\"/.gemini/hooks/log-reminder.py",
          "timeout": 10
        }]
      }
    ]
...
**Permission modes.** Gemini CLI has five permission modes that control how much autonomy Gemini gets:
...
Set via CLI flag (`gemini --permission-mode plan`), the `/config` command, or `permissions.defaultMode` in `settings.json`.

### The Six-Layer Permission Stack

Permission mode is not resolved from a single file. Gemini CLI honors **six layers**, with later layers overriding earlier ones:

| # | Layer | Location | Key |
|---|---|---|---|
| 1 | VSCode user | `~/Library/Application Support/Code/User/settings.json` (macOS), `~/.config/Code/User/settings.json` (Linux), `%APPDATA%/Code/User/settings.json` (Windows) | `geminiCode.initialPermissionMode` |
| 2 | VSCode workspace | `<repo>/.vscode/settings.json` | `geminiCode.initialPermissionMode` |
| 3 | CLI user | `~/.gemini/settings.json` | `permissions.defaultMode` |
| 4 | CLI project | `<repo>/.gemini/settings.json` | `permissions.defaultMode` |
| 5 | CLI project-local (gitignored) | `<repo>/.gemini/settings.local.json` | `permissions.defaultMode` |
...
1. **Look at the status line** at the top of the Gemini CLI panel. With this repo's `statusLine` configured, it prints `[BYPASS]`, `[PLAN]`, `[AUTO-EDIT]`, or `[PROMPT]` for the four standard modes. Any other mode Gemini CLI reports (e.g., `dontAsk`) is shown as a bracketed raw name like `[dontAsk]`. If it doesn't say `[BYPASS]`, you have an in-session override --- press `Shift+Tab` to cycle modes until you land on bypass.
...
5. **VSCode vs CLI split.** If the VSCode layers say bypass but no CLI layer does, a terminal-launched Gemini CLI will still prompt. Align them.
...
```
Start session in plan mode  (Shift+Tab at startup, or gemini --permission-mode plan)
     |
...
User approves via ExitPlanMode UI
     |
     | Gemini CLI returns the session to defaultMode.
...
By default, Gemini saves plans to a global directory (`~/.gemini/plans/`), not your project. To keep plans with your project (and in git), add this to `.gemini/settings.json`:
...
The **Stop hook** runs a fast Python script after every response. No LLM call, no latency. It checks whether the session log is current and reminds Gemini to update it if not. Behavioral rules like verification and Beamer-Quarto sync are enforced via auto-loaded rules in `.gemini/rules/`, which is the right tool for nuanced judgment that Gemini can evaluate in-context.

## Effort Levels --- Cost vs. Thoroughness {#sec-effort}

Gemini CLI lets you control how deeply it reasons about each task. Higher effort means more "thinking tokens" and better results --- but higher cost.
...
Gemini CLI has an auto-memory system at `~/.gemini/projects/[project]/memory/MEMORY.md`. This file persists across sessions and is loaded into every conversation.
...
The session log reminder above is one example of a broader pattern: using **hooks** to enforce rules that Gemini might otherwise forget during long sessions. Rules live in context and can be compressed away. Hooks live in `.gemini/settings.json` and fire every time, regardless of context state.
...
**Hook handler types.** The examples above use `command` hooks (shell scripts), but Gemini CLI supports four handler types:
...
Now that you understand the building blocks, here's what's new: Gemini CLI has added [effort levels](#sec-effort) for cost control, [expanded hook events](#hooks---automated-enforcement), [skill frontmatter fields](#skill-frontmatter) for fine-grained skill configuration, [permission modes](#settings---permissions-and-hooks) for controlling Gemini's autonomy, [advanced agent configuration](#sec-agent-config), plugins, `/batch` for parallel refactoring, and headless CLI mode. Each is covered in its respective section above.
:::
...
Phase 2: CHECK EXISTING
  Search .gemini/skills/ for related skills
  → Nothing related? Create new. Overlap? Update existing.
         ↓
Phase 3: CREATE SKILL
  Write to .gemini/skills/[name]/SKILL.md
  • Problem statement
...
Skills saved to `.gemini/skills/` survive compaction and session boundaries --- if you discover something valuable late in a session, extract it with `/learn` before compaction erases the details.
...
Gemini CLI can spawn **multiple agents simultaneously** using the Task tool. This is not limited to review --- you can use it for any research or analysis task where independent subtasks can run at the same time.
...
The **kill switch** is explicit: at any point, you can stop, archive with a one-paragraph explanation, and move on. No guilt, no sunk cost. See `.gemini/rules/exploration-folder-protocol.md` and `.gemini/rules/exploration-fast-track.md` for the full protocols.

#### Simplified Orchestrator for Research

The full orchestrator (Pattern 2) is designed for course materials with multi-agent review loops. For research projects, the **simple variant** strips this down to: implement → verify → score → done. No multi-round reviews, no parallel agent spawning. This lives in its own path-scoped rule (`.gemini/rules/orchestrator-research.md`) that loads only when working on R scripts or explorations.

#### Merge-Only Quality Reporting

In research projects, commits are frequent and incremental. Generating a quality report for each commit creates noise. Instead, quality reports are generated **only at merge time** --- a permanent snapshot of what was merged and why. Session logs capture the ongoing reasoning. See `.gemini/rules/session-logging.md`.
...
The `/slide-excellence` skill already invokes the pedagogy-reviewer and slide-auditor, which enforce many of these principles automatically. To enforce *all* of them --- including title-as-assertion and MB/MC smoothness --- customize the **domain-reviewer** agent (`.gemini/agents/domain-reviewer.md`) with rhetoric-of-decks lenses. The orchestrator will then apply them during every review cycle without manual invocation.

::: {.callout-note collapse="true"}
## The Full Framework

For the complete philosophical treatment --- from Aristotle's three modes of persuasion through neuroaesthetics and the Netflix analogy --- see [The Rhetoric of Decks](https://github.com/scunning1975/MixtapeTools/tree/main/presentations). The repository includes a full essay, example Beamer decks with professional color palettes, a `theme_rhetoric()` ggplot2 theme, and a tested deck generation prompt for Gemini CLI.
:::
...
| Voice preservation | geminiblattman | Maintain a reference doc with your writing style; load as context |
| Template-executor separation | Xu & Yang | Spec = what to measure, orchestrator = how to execute |
| Self-improving configuration | geminiblattman | Use `/learn` to capture discoveries; review MEMORY.md periodically |
...
| Sequential independent audits | GeminiCodeTools | Run N blind audit passes; independence prevents groupthink ([Pattern 15](#pattern-15-sequential-audits)) |

::: {.callout-tip}
## Which Ecosystem Project Should I Start With?

If you write **papers**: start with clo-author (adversarial review pairs) or GeminiCodeTools (seven-audit protocol, see [Pattern 15](#pattern-15-sequential-audits)). If you give **presentations**: MixtapeTools. If you run **computational experiments**: autoresearch. If you're a **non-technical academic**: geminiblattman. All of them build on the same foundation patterns from this template — the [orchestrator](#sec-system), [quality gates](#sec-quality), and [adversarial review](#the-adversarial-pattern-critic-fixer).
:::
...
## geminiblattman: Workflows for Non-Technical Academics

**Website:** [geminiblattman.com](https://geminiblattman.com)
**Repository:** [chrisblattman/geminiblattman](https://github.com/chrisblattman/geminiblattman)
**Author:** Chris Blattman (University of Chicago)

geminiblattman is a comprehensive guide for academics who do not write code, built by a political economist who describes himself as someone who "has never written a line of code." It demonstrates that Gemini CLI workflows extend far beyond technical tasks into daily academic life.
...
**When to use it:** You are new to Gemini CLI, want practical daily workflows beyond coding, or want to see how an academic non-programmer built a sophisticated system.
...
**When to use it:** You want to make your presentations genuinely beautiful and rhetorically effective, or you want a tested deck generation workflow for Gemini CLI.
...
A collection of Gemini CLI personas, including "The Editor" --- a deeply structured academic paper reviewer that runs seven sequential audit passes (see [Pattern 15](#pattern-15-sequential-audits)). Notable features:
...
Copy `.gemini/agents/domain-reviewer.md` and customize the [5-lens framework](#sec-domain-reviewer) for your field. The template provides the structure; you fill in domain-specific checks.
...
Each skill is a directory in `.gemini/skills/` with a `SKILL.md` file:
...
The `description` field is the single most important thing you write when creating a skill. Gemini reads the description of every available skill on every turn and uses it to decide whether to auto-invoke without the user typing a slash command. A vague description means a skill that only runs when the user remembers its name --- defeating the point of auto-invocation.
...
1. **Copy the template:**
   ...
   cp templates/skill-template.md .gemini/skills/your-skill-name/SKILL.md
   ```
...
9. **Use `CLAUDE.local.md` for personal overrides.** This file is automatically gitignored and loaded alongside `GEMINI.md`. Put machine-specific paths, personal preferences, and local tool versions here --- they won't pollute the shared repo.

::: {.callout-note collapse="true"}
## Extending with MCP Servers

For capabilities beyond file editing and shell commands --- web search during literature review, database queries for replication, or reference manager integration (Zotero, Mendeley) --- Gemini CLI supports [MCP servers](https://modelcontextprotocol.io). Configure them in `.gemini/settings.json` under `"mcpServers"`. Start with skills and agents first; add MCP when you need external integrations.

## Extending with Plugins

Gemini CLI supports **plugins** --- bundled collections of skills, agents, hooks, and MCP servers that can be installed from git repositories. Use `/plugin` to browse and manage plugins (it has a Discover tab for finding new ones). Plugins are a newer extension point; start with skills and rules (which you control entirely) before adopting third-party plugins.
:::
...
| Agent | File | Purpose |
|-------|------|---------|
| Proofreader | `.gemini/agents/proofreader.md` | Grammar, typos, consistency |
| Slide Auditor | `.gemini/agents/slide-auditor.md` | Visual layout, overflow, spacing |
| Pedagogy Reviewer | `.gemini/agents/pedagogy-reviewer.md` | Narrative arc, notation clarity |
| R Reviewer | `.gemini/agents/r-reviewer.md` | R code quality, reproducibility |
| TikZ Reviewer | `.gemini/agents/tikz-reviewer.md` | Diagram visual quality |
| Beamer Translator | `.gemini/agents/beamer-translator.md` | LaTeX to Quarto translation |
| Quarto Critic | `.gemini/agents/quarto-critic.md` | Adversarial Quarto QA |
| Quarto Fixer | `.gemini/agents/quarto-fixer.md` | Applies critic's fixes |
| Verifier | `.gemini/agents/verifier.md` | Task completion verification |
| Domain Reviewer | `.gemini/agents/domain-reviewer.md` | Your domain-specific review |
| Claim Verifier | `.gemini/agents/claim-verifier.md` | Chain-of-Verification (fresh-context) fact-checker (v1.7.0) |

## All Skills

| Skill | Directory | Purpose |
|-------|-----------|---------|
| `/compile-latex` | `.gemini/skills/compile-latex/` | XeLaTeX 3-pass compilation |
| `/deploy` | `.gemini/skills/deploy/` | Quarto render + GitHub Pages sync |
| `/extract-tikz` | `.gemini/skills/extract-tikz/` | TikZ to SVG conversion |
| `/new-diagram` | `.gemini/skills/new-diagram/` | Scaffold a TikZ diagram from the gallery |
| `/proofread` | `.gemini/skills/proofread/` | Run proofreading agent |
| `/visual-audit` | `.gemini/skills/visual-audit/` | Run layout audit agent |
| `/pedagogy-review` | `.gemini/skills/pedagogy-review/` | Run pedagogy review agent |
| `/review-r` | `.gemini/skills/review-r/` | Run R code review agent |
| `/qa-quarto` | `.gemini/skills/qa-quarto/` | Critic-fixer adversarial loop |
| `/slide-excellence` | `.gemini/skills/slide-excellence/` | Combined multi-agent review |
| `/translate-to-quarto` | `.gemini/skills/translate-to-quarto/` | Beamer to Quarto translation |
| `/validate-bib` | `.gemini/skills/validate-bib/` | Bibliography validation |
| `/devils-advocate` | `.gemini/skills/devils-advocate/` | Design challenge questions |
| `/create-lecture` | `.gemini/skills/create-lecture/` | Full lecture creation |
| `/commit` | `.gemini/skills/commit/` | Stage, commit, PR, and merge |
| `/lit-review` | `.gemini/skills/lit-review/` | Literature search and synthesis |
| `/research-ideation` | `.gemini/skills/research-ideation/` | Research questions and strategies |
| `/interview-me` | `.gemini/skills/interview-me/` | Interactive research interview |
| `/review-paper` | `.gemini/skills/review-paper/` | Manuscript review |
| `/respond-to-referees` | `.gemini/skills/respond-to-referees/` | R&R cross-reference and response draft |
| `/data-analysis` | `.gemini/skills/data-analysis/` | End-to-end R analysis |
| `/audit-reproducibility` | `.gemini/skills/audit-reproducibility/` | Enforce replication tolerance thresholds on paper ↔ code |
| `/learn` | `.gemini/skills/learn/` | Extract discoveries into persistent skills |
| `/context-status` | `.gemini/skills/context-status/` | Show session health and context usage |
| `/deep-audit` | `.gemini/skills/deep-audit/` | Repository-wide consistency audit |
| `/seven-pass-review` | `.gemini/skills/seven-pass-review/` | Seven-pass adversarial manuscript review (parallel forked subagents) |
| `/permission-check` | `.gemini/skills/permission-check/` | Diagnose permission layers (6-tier stack) |
| `/verify-claims` | `.gemini/skills/verify-claims/` | Chain-of-Verification fact-check on any draft (v1.7.0) |
...
|------|------|--------------|
| Session log reminder | Stop (command) | `.gemini/hooks/log-reminder.py` |
| Desktop notification | Notification (command) | `.gemini/hooks/notify.sh` |
| Context state capture | PreCompact (command) | `.gemini/hooks/pre-compact.py` |
| Context restoration | SessionStart[compact\|resume] (command) | `.gemini/hooks/post-compact-restore.py` |
| Context monitor | PostToolUse[Bash\|Task] (command) | `.gemini/hooks/context-monitor.py` |
| Verification reminder | PostToolUse[Write\|Edit] (command) | `.gemini/hooks/verify-reminder.py` |

**Additional hook events** available in Gemini CLI (not used in this template but available for custom hooks):
...
**Fix:**
1. Check hooks are configured: `cat .gemini/settings.json | grep hooks`
2. Ensure Python 3 is available: `which python3`
3. Check hook file permissions: `ls -la .gemini/hooks/`

### Gemini Ignores Rules

**Symptom:** Gemini doesn't follow conventions in `.gemini/rules/`.
...
3. Try: *"Read `.gemini/rules/[rule].md` and follow it for this task"*
...
**Symptom:** Plans save to `~/.gemini/plans/` instead of your project directory. Can't track plans in git.
...
This tells Gemini CLI to save plans inside your project where they can be version-controlled.
...
- [Gemini CLI](https://code.gemini.com/docs/en/overview) by Anthropic --- the CLI tool, VS Code extension, and Desktop app that makes all of this possible
...
- [geminiblattman](https://github.com/chrisblattman/geminiblattman) by Chris Blattman (University of Chicago) --- comprehensive workflows for non-technical academics: executive assistant, proposal writing, project management, and the fresh-context critique pattern
